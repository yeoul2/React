...
import { fetchRecommendRoute, fetchRoute } from "../../services/googlePlacesService";
...
const calculateTravelDurations = async (index, scheduleList) => {
   ...
   try {
      const drivingRes = await fetchRoute(origin, destination, "driving");
      const transitRes = await fetchRoute(origin, destination, "transit");
