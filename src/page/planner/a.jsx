...
import TravelSearch from "../../components/TravelSearch"; // TravelSearch 컴포넌트 import
...
const PlannerPage = () => {
  ...
  const peopleDropdownRef = useRef(null); // 📌 드롭다운 요소 참조
  ...
  const [isCountryOpen, setIsCountryOpen] = useState(false); // 📌 국가 입력 필드 열림/닫힘 상태
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (peopleDropdownRef.current && !peopleDropdownRef.current.contains(event.target)) {
        setIsPeopleOpen(false); // 📌 드롭다운 영역 밖을 클릭하면 닫기
      }
    };
    ...
    return () => {
      document.removeEventListener("mousedown", handleClickOutside); // 📌 컴포넌트 언마운트 시 정리
    };
  }, [isPeopleOpen]);
      ...
      <div className="bg-white shadow sm:rounded-lg p-6 mb-12">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3 mb-3">
          ...
          <div>
            ...
            {/* 📌 인원 선택 드롭다운 */}
            {isPeopleOpen && (
              <div ref={peopleDropdownRef} className="border border-gray-300 mt-2 rounded-lg p-4 shadow-lg bg-white">
                {/* 성인 선택 */}
