...
import { fetchAutocomplete } from '../../services/googlePlacesService';
...
const PlannerPage = () => {
  ...
  // AI 일정 생성
  const generateScheduleFromAI = async () => {
    try {
      const response = await axios.post("http://localhost:5005/generate-schedule", {
        city: selectedCity,
        days: travelDays,
        people: numberOfPeople,
        style: selectedThemes.join(", "), // 선택된 테마들을 join해서 전달
      });

      const aiSchedule = response.data.schedule;
      setAiSchedule(aiSchedule); // 상태에 AI 일정 저장
    } catch (error) {
  ...
  const handleConfirmSearch = async () => {
    ...
    try {
      ...
      // ✅ 2. AI 일정 생성 요청 (FastAPI 백엔드)
      setIsLoading(true);

      const aiResponse = await axios.post(`${process.env.REACT_APP_FASTAPI_URL}/generate-plan`, {
        country: searchTerm,                             // ✅ 나라 or 도시
        days: travelDays,                                // ✅ 기존: 3 → 사용자가 선택한 기간 반영
        members: adults,                                 // ✅ 인원수
        style: travelStyle.join(", ")                    // ✅ 추가: 선택한 테마 함께 전달
      });

      // ✅ 3. 추천 일정 결과 처리
      const planData = aiResponse.data?.plan || [];
      setPlans(planData); // 추천 일정 렌더링

      // ✅ 4. 지도에 표시할 마커 위치 추출 (위경도 포함돼 있어야 함)
      ...
      console.error("❌ 확인 버튼 처리 실패:", error);
      setPlans([]); // ✅ 렌더링 오류 방지
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGenerateAIPlan = async () => {
    ...
    const requestData = {
      ...
      people: adults || 2, // 사용 중인 인원 수 상태
      style: travelStyle.join(", ")
    };

    try {
      // ✅ 1. AI 일정 요청 (Spring → FastAPI)
      const response = await axios.post("/api/ai/generate-schedule", requestData);

      const aiData = response.data; // AI가 생성한 일정 배열
      if (!aiData || !Array.isArray(aiData)) throw new Error("AI 일정이 유효하지 않음");
      ...
      if (markers.length > 0) {
        // ✅ 지도 중심 좌표 업데이트
        setMapCenter({ lat: markers[0].lat, lng: markers[0].lng });
        // setMapMarkers(markers); // 마커 상태에 저장
      }
    } catch (error) {
      console.error("❌ AI 일정 생성 실패:", error);
